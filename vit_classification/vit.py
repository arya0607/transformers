...
class ViT(nn.Module):
    ...
    def __init__(self, patch_dim, d_model, d_ff, num_heads, num_layers, num_patches, num_classes, device = 'cuda'):
        ...
        self.patch_embedding = nn.Linear(self.patch_dim **2 * 3, d_model) # TODO (Linear Layer that takes as input a patch and outputs a d_model dimensional vector)
        ...
        self.cls_token = nn.Parameter(torch.zeros(1,1,self.d_model)).to(self.device) # TODO (learnable [CLS] token embedding)
    ...
    def patchify(self, images):
        ...
        patches = nn.Unfold(kernel_size=self.patch_dim, stride=self.patch_dim)(images)
