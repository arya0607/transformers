...
class MultiHeadAttentionLayer(AttentionLayer):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0.1):
        ...
        # TODO: Initialize the following layers and parameters to perform attention
        self.head_proj = nn.Linear(self.embed_dim, embed_dim)
        # self.key_proj = nn.Linear(embed_dim, embed_dim)
        # self.value_proj = nn.Linear(embed_dim, embed_dim)
        # self.query_proj = nn.Linear(embed_dim, embed_dim)
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        dot_product = query @ key.transpose(2, 3) / math.sqrt(self.head_dim)
...
class SelfAttentionBlock(nn.Module):
    ...
    def forward(self, seq, mask):
        ...
        attn = self.self_attn(seq, seq, seq, mask)
        out = self.layernorm(seq + self.dropout(attn))
        return out
...
class CrossAttentionBlock(nn.Module):
    ...
    def forward(self, seq, cond):
        ...
        # Then add a residual connection to the original input, and finally apply normalization. #############################
        attn = self.cross_attn(seq, cond, cond)
        out = self.norm(seq + self.dropout(attn))
        return out
...
class FeedForwardBlock(nn.Module):
    ...
    def forward(self, seq):
        ...
        # Then add a residual connection to the original input, and finally apply normalization. #############################
        mlp = self.mlp(seq)
        out = self.norm(seq + self.dropout(mlp))
        return out
...
class TransformerDecoder(nn.Module):
    ...
    def get_causal_mask(self, _len):
        ...
        mask = torch.ones(_len, _len).to(self.device)
